type ArgFreeEventType = "selectionchange" | "currentpagechange" | "close"
type Constructor<T = {}> = new (...args: any[]) => T;

type ManifestMenuItem =
  // Clickable menu item.
  { name: string, command: string, parameters?: ParameterValues[], parameterOnly?: boolean } |
  // Separator
  { separator: true } |
  // Submenu
...
interface NotificationOptions {
  timeout?: number // 通知在关闭前保持的时间，以 ms 为单位，默认 4000ms
  error?: boolean // 是否展示为错误信息
}
...
interface ImageFilters {
  readonly exposure?: number // 曝光
  readonly contrast?: number // 对比度
  readonly saturation?: number // 饱和度
  readonly temperature?: number // 色温
  readonly tint?: number // 着色
  readonly highlights?: number // 高光
  readonly shadows?: number // 阴影
}
...
interface BaseFrameMixin extends
  ...
  gridStyleId: string
  clipsContent: boolean // frame 内的图层在 frame 外是否裁剪
...
interface PageNode extends BaseNodeMixin, ChildrenMixin, ExportMixin {
  ...
  guides: ReadonlyArray<Guide> // jsDesign.guides 每次被调用都会创建一个新的数组
